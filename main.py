...
if 'VERCEL' in os.environ:
    ...
    DEBUG = False
    # Use /tmp directory for SQLite on Vercel
    DATABASE_PATH = '/tmp/prepboosters.db'
else:
    # Local development settings
    HOST = 'localhost'
    PORT = 5000
    DEBUG = True
    DATABASE_PATH = 'prepboosters.db'
...
def initialize_database():
    with app.app_context():
        try:
            ...
            # Create backup admin
            if not User.query.filter_by(username='backup_admin').first():
                backup_admin = User(
                    username='backup_admin',
                    email='backup@prepboosters.com',
                    password=generate_password_hash('backup123'),
                    is_admin=True
                )
...
def admin_books():
    try:
        ...
        return render_template('admin/books.html', books=[], categories=[])
...
def delete_book(book_id):
    try:
        ...
        flash('Failed to deactivate book', 'danger')
...
def clear_chat():
    try:
        ...
        flash('Failed to clear chat', 'danger')
...
def make_admin(user_id):
    try:
        ...
        flash('Failed to make user admin', 'danger')
...
def backup_database():
    try:
        ...
        flash('Backup failed. Database might be read-only on this server.', 'warning')
...
def check_username(username):
    try:
        ...
        return jsonify({'available': True, 'error': 'Database unavailable'})
    ...
    try:
        ...
        for msg in messages:
            messages_data.append({
                ...
                'user_id': msg.user_id,
                'created_at': msg.created_at.strftime('%H:%M') if msg.created_at else '00:00',
                'is_current_user': msg.user_id == session['user_id']
            })
        
        return jsonify({'messages': messages_data})
    except:
        return jsonify({'messages': [], 'error': 'Failed to load messages'})
...
def online_users():
    try:
        ...
        for user in online_users_list:
            mins_ago = (datetime.utcnow() - user.last_active).seconds // 60 if user.last_active else 999
...
if __name__ == '__main__':
    ...
    directories = ['templates', 'templates/admin', 'static', 'static/css', 'static/js', 'static/images']
...
<body>
    ...
    print("\n" + "="*50)
    print("🚀 PrepBoosters Website")
    print("="*50)
    print(f"🌐 Running on: {HOST}:{PORT}")
    print("🔑 Admin: username='admin', password='admin123'")
    print("🔑 Backup: username='backup_admin', password='backup123'")
    print("="*50 + "\n")
